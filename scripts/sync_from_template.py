...
NEVER = (
    ...
    # 순서의 SSOT. 목표에서 역산한 경로와 `[완료]` 표시가 여기 쌓인다 — 학습 자산이다.
    "learning-spec.md",
    "courses/",  # 내가 들은 과목 노트 — 설계도의 한 단계를 공부할 재료다
...
SEED_IF_MISSING = (
    ...
    # `learning-spec.md`는 **여기에 넣지 않는다.** `materials/READING_STATUS.md`와 같은
    # 전례를 따른다 — 견본을 미리 놓지 않고 CI가 첫 사용 때 만든다(`ensure_learning_spec`).
    # 견본 파일을 두면 템플릿과 파이썬 상수에 같은 텍스트가 두 벌 생긴다. 그리고 파일이
    # 없는 상태가 곧 "아직 목표를 안 정했다"는 사실이라, 미리 놓으면 그것이 가려진다.
)
