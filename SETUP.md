...
그러면 GPT가:
1. 내 `STATUS.md`를 읽고 (아직 템플릿이면 **목표를 묻습니다** — 새로 만든 repo의 STATUS는
   비어 있는 게 아니라 **예시가 채워져 있습니다.** GPT는 그걸 예시로 알아보고 넘어갑니다)
...
## 6단계 — 논문 자동 수집 켜기 (2분, 논문 트랙이면 사실상 필수)

연구 주제를 따라가는 중이라면, **매주 새 논문**을 자동으로 모아 줍니다.

> ⚠️ 템플릿의 기본값은 **꺼짐**(`topics: []`)입니다. 켜기 전까지 `papers/inbox.md`는 **계속 0편**입니다.
> 가장 쉬운 길: Topdown **시작하기(/start)** 의 **④ 버튼** — 위저드가 `topics.yaml`을
> (영어 검색어로) 만들어 주고 편집 화면까지 열어 줍니다. 붙여넣고 커밋하면 끝.
...
> 📄 논문을 다루는 나머지 경로(시작 5편 · 세션 중 검색 · **PDF 정독 세션**)와
> *"Paper GPT는 어디 있나"* 는 **[`PAPERS.md`](./PAPERS.md)** 에 정리돼 있습니다.

---

## 7단계 (선택) — 그래프로 내 상태 보기

세션을 몇 번(개념 10개쯤) 쌓으면 `concepts.json`이 자동으로 만들어집니다.
그러면 **Topdown 앱의 `/learning-repo`** 에 내 저장소 이름(`내아이디/my-learning`)만 넣으면:

- 개념 상태 분포 (설명가능 / 암기 / 미학습)
- 🚧 **막힌 길목** — 아직 설명 못 하는데 **다른 개념을 막고 있는** 것

을 볼 수 있습니다. 로그인 없이 읽기만 합니다.
...
**Q. GPT가 "저장소에 연결할 수 없다 / 연결될 수 없는 환경이다" 라고 합니다.** ← 추측하지 말고 아래 순서로

> 💡 **가장 흔한 원인은 연결이 아니라 내용입니다.** 새로 만든 repo의 `STATUS.md`는 비어 있는 게
> 아니라 **예시(`<트랙 이름 …>`)가 채워져 있어서**, 파일은 정상적으로 읽혔는데도 GPT가
> "제대로 된 상태가 아니다 → 진행할 수 없는 환경"이라고 잘못 말하는 일이 있습니다.
> **이건 연결 문제가 아닙니다.** 첫 세션에서 목표를 말하면 그 세션 기록으로 STATUS가
> 실제 값으로 바뀌고, 다음부터는 안 그럽니다. 그래도 이상하면 아래로 내려가세요.

이 말은 원인이 **최소 4가지**입니다. 순서대로 하면 30초 안에 어느 것인지 갈립니다.

**① 액션이 실제로 붙었는지** — GPT에게 그대로 물어보세요:
```
네가 지금 쓸 수 있는 액션(도구) 이름을 그대로 나열해.
```
`readFile · createNote · appendNote · closeNote` **4개가 안 나오면 액션이 안 붙은 것**입니다
(4단계를 다시). 지침·프롬프트 문제가 아니므로 말투를 바꿔도 소용없습니다.
> 일반 ChatGPT 대화창에 지침만 붙여넣은 경우도 여기 해당합니다 — **토큰을 붙여넣어도
> 대화창은 GitHub에 요청을 보낼 수단 자체가 없습니다.** 반드시 Custom GPT의 Action이어야 합니다.

**② 4개가 나오는데 실패한다면 — 호출을 강제하고 원문을 보세요:**
```
지금 readFile 액션으로 내 repo(내아이디/my-learning)의 STATUS.md를 실제로 호출하고,
성공/실패와 상관없이 **응답 원문과 상태 코드를 그대로** 보여줘. 요약하지 마.
```
ChatGPT 화면의 액션 호출 카드를 **펼치면** 실제 요청 URL과 응답이 보입니다. 그 화면을 캡처하세요.

**③ 상태 코드로 원인 확정:**

| 응답 | 원인 | 고치는 법 |
|---|---|---|
| **401** `Bad credentials` | 토큰 값이 틀림/만료 | PAT 재발급 → GPT Actions의 인증값 교체. **Auth Type이 Bearer**인지 확인 |
| **403** | 권한 부족 | PAT의 **Contents=Read-only, Issues=Read and write** 확인 |
| **404** | 가장 흔함 — repo를 못 봄 | ⓐ `내아이디/repo이름` 오타 ⓑ PAT의 **Repository access**에 그 repo 미선택 ⓒ **fine-grained PAT은 발급한 계정이 소유한 repo만** 접근됩니다 — 조직(organization) 소속으로 만들었다면 Resource owner를 그 조직으로 잡고 조직 승인이 필요합니다 |
| 긴 base64 덩어리 | `Accept` 헤더 미전송 | 세션 중 *"Accept를 application/vnd.github.raw로 보내"* 라고 지시. 반복되면 4단계에서 스키마를 다시 붙여넣기 |
| 호출 흔적 자체가 없음 | 모델이 시도조차 안 함 | 위 ②처럼 **"실제로 호출해"** 라고 명령. 그래도 안 하면 ①로 |

**④ GPT 문제인지 토큰 문제인지 10초 만에 가르기** (터미널이 있다면):
```bash
curl -i -H "Authorization: Bearer <내 PAT>" \
     -H "Accept: application/vnd.github.raw" \
     https://api.github.com/repos/내아이디/my-learning/contents/STATUS.md
```
- **200 + 파일 내용** → 토큰은 정상. 문제는 **GPT 설정(①②)** 쪽입니다.
- **401/403/404** → 토큰·권한 문제. 위 표대로 고칩니다.
