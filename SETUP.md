...
4. 세션이 끝나면 `save_session` **한 번**으로 기록합니다.
...
| 막힌 개념 집중 | `막힌 길목부터 하자.` |
| 세션 마무리 | `save_session으로 오늘 세션 기록해줘.` |

> 예전 세팅에서 필요했던 **`STATUS.md 읽고`** 같은 말머리는 이제 필요 없습니다.
> 그냥 "오늘 세션 시작"이면 AI가 알아서 불러옵니다.

> ⚠️ **마무리할 때만 도구 이름을 붙여 주세요.** GitHub 커넥터를 함께 켜 두었다면
> 그냥 "오늘 세션 저장"이라고 했을 때 ChatGPT가 GitHub 쪽으로 새서 **형식이 다른 Issue**를
> 만들어 버립니다. 그 Issue는 자동화가 읽지 못해 노트 파일이 만들어지지 않습니다 —
> 저장된 것처럼 보이지만 기록은 없습니다. `save_session으로` 한 마디가 그걸 막습니다.
