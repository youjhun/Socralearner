...
3. 칸을 채웁니다:
   ...
   - **⚠️ 그리고 붙여넣은 것 맨 아래에 `# 이 학습자` 블록을 이어 붙입니다** — 이게 없으면
     GPT는 내 저장소가 어딘지 모릅니다. Custom GPT는 대화가 바뀌면 기억이 없어서,
     **새 대화를 열 때마다 "owner/repo를 알려주세요"를 다시 묻습니다.**

     ```
     # 이 학습자
     - 저장소: 내아이디/my-learning
     - 호칭: (이름)
     - 대상: (예: 학부 3학년 전자공학 / 랩 인턴 준비)
     - 읽기 방식: (예: 답을 먼저 주지 말 것. 개념 복원 위주.)
     ```

     저장소 줄 하나만 있어도 됩니다. 나머지는 있으면 매 세션 다시 설명하지 않아도
     되는 것들이고, 지침 대본보다 **여기 적힌 것이 우선합니다**.
...
> 4단계에서 `# 이 학습자` 블록에 저장소를 적었다면 이것으로 충분합니다. GPT가
> `owner/repo`를 되물으면 그 블록이 없거나 안 붙은 것입니다 — 4-3으로 돌아가세요.
> (`내 repo는 …이야`라고 매번 알려 줘도 되지만, Custom GPT는 대화가 바뀌면 기억이
> 없어서 **새 대화마다** 다시 말해야 합니다.)
